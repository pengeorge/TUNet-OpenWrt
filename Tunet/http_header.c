...
int http_header_add_post_field(http_header_t* header, char* field_name,
		char* field_value) {
	if (header->post_field_count >= MAX_HTTP_HEADER_FIELD_COUNT){
		return -1;
	}
	header->post_field_name[header->post_field_count] = strndup(
			field_name, MAX_HTTP_HEADER_FIELD_LENGTH);
	header->post_field_value[header->post_field_count] = strndup(
			field_value, MAX_HTTP_HEADER_FIELD_LENGTH);
...
int http_header_to_text(http_header_t* header, char* output, int output_size) {
	...
	for (int i = 0; i < header->request_field_count; i++) {
		...
		offset += 2;
		strncpy(output + offset, header->request_field_value,
				MAX_HTTP_HEADER_FIELD_LENGTH);
		offset += value_length;
	...
	if (HTTP_REQUEST_TYPE_POST == header->http_request_type) {
		strncpy(output + offset, "\r\n", MAX_HTTP_HEADER_FIELD_LENGTH);
		offset += 2;
		for (int i = 0; i < header->post_field_count; i++) {
			if (i != 0) {
				strncpy(output + offset, "&", MAX_HTTP_HEADER_FIELD_LENGTH);
				offset += 1;
			}
...
int http_header_test(){
	...
	http_header_set_request_type(HTTP_REQUEST_TYPE_POST);
